@php
    ...
    if ($user) {
        if ($user->tipo === 'admin' && Route::has('admin.dashboard')) {
            $dashboardRoute = route('admin.dashboard');
        } elseif ($user->tipo === 'aluno' && Route::has('aluno.dashboard')) {
            $dashboardRoute = route('aluno.dashboard');
        }
    }
@endphp
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    @php
                        $dashboardRoute = auth()->user()->tipo_usuario === 'admin' 
                            ? 'admin.dashboard' 
                            : 'aluno.dashboard';
                    @endphp
            ...
            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                <x-nav-link 
                    :href="$dashboardRoute" 
                    :active="request()->routeIs('admin.dashboard') || request()->routeIs('aluno.dashboard')"
                >
                    {{ __('Dashboard') }}
                </x-nav-link>
            </div>
